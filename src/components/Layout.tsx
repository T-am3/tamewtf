    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <nav className={`fixed top-0 w-full z-40 transition-all duration-300 ${
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16 relative">
            ...
            <Link 
              to="/" 
              className="text-lg text-white hover:text-gray-300 transition-colors font-medium tracking-wide flex items-center h-full"
            >
              tame.wtf
            </Link>
            ...
            <div className="hidden md:flex items-center space-x-8">
              {navLinks.map(({ path, label }) => (
                ...
                >
                  {label}
                  {location.pathname === path && (
                    <span className="absolute -bottom-2 left-0 w-full h-0.5 bg-white animate-in slide-in-from-left duration-300" />
                  )}
                </Link>
              ))}
              ...
              <button
                ...
                className="w-8 h-8 rounded-lg bg-gray-800/50 border border-gray-700/50 flex items-center justify-center hover:bg-gray-700/50 transition-all duration-200 group"
                title="Theme toggle"
              >
        ...
        <div className={`md:hidden transition-all duration-300 overflow-hidden ${
          ...
          <div className="bg-black backdrop-blur-lg border-t border-gray-800">
      ...
      {showThemeAlert && (
        <div className="fixed top-20 right-4 z-50 animate-in slide-in-from-right duration-300">
          <div className="glass rounded-lg p-6 shadow-2xl max-w-sm hover-lift">
            <div>
              <h3 className="text-white font-medium mb-2">lol im not implementing that</h3>
              <p className="text-gray-400 text-sm leading-relaxed">
                this site is already perfectly dark, what more do you want?
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-8">
            <div>
              ...
              <div className="space-y-2">
                <a href="mailto:hello@tame.wtf" className="block text-gray-400 hover:text-white transition-colors text-sm">
                  hello@tame.wtf
                </a>
              </div>
            </div>
            <div>
              <h3 className="text-white font-medium mb-4">work</h3>
              <div className="space-y-2">
                <Link to="/work" className="block text-gray-400 hover:text-white transition-colors text-sm">
                  visual effects
                </Link>
                <Link to="/work" className="block text-gray-400 hover:text-white transition-colors text-sm">
                  music production
